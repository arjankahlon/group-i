...
thresh = -100
avgthresh = -110
calthresh = 0.00000000010
avgcalthresh = 0.075 #TODO
...
plotflag = 0
network_scale = 0.2 # set to 1 for full scale or 0.2 for a quick test with a small network #TODO
...
def occasional_fcn():
    ...
    for cell in cells: #Check Voltage list, if the cell needs to die.
        if (cell.gid>=ncell):
            break
        cell_dies = 0
        cell_dies = checkifcelldies(np.asarray(results["cellv_" + str(cell.gid)])[int(h.t//h.dt)-300::],thresh,avgthresh)
...
def veryinterestingoccasional_fcn():
    for cell in cells:
        if(cell.gid>= ncell):
            break
        more_Ampa = 0
        more_Ampa = checkAMPAr(np.asarray(results["celli_" + str(cell.gid)])[int(h.t//h.dt)-75::],calthresh, avgcalthresh)
        
        if more_Ampa == 1:
            for syn in cell.list_syns:
                syn.weight[0] += 0.05 #TODO not sure weight Model limitation, what if Ca conc is too little?
                
                
            
        
    h.cvode.event(h.t+veryinterestingStepBy, veryinterestingoccasional_fcn)

fihw = h.FInitializeHandler(2, occasional_fcn) # run it at the start of the sim
fihe = h.FInitializeHandler(2, veryinterestingoccasional_fcn) # run it at the start of the sim
...
def checkifcelldies(V_overtime,thresh,avgthresh):
